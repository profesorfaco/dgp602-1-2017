...
<p>Pero si reducimos el listado recién presentado a los que "han demostrado una actividad académica sostenida, capacidad y aptitudes para realizarla en forma autónoma y creativa y dominio de su especialidad", sólo tenemos 2 profesores, quienes además son autores de textos de uso docente:</p>
<ol>
<?php
for ($d = 0; $d < $all; $d++) {
  if((($datos[$d]['Rango']) == "Profesor Asociado") && ($datos[$d]['Horas'])==44){
    echo '<li>'.strstr($datos[$d]["Nombres"], ' ', true).' '.$datos[$d]["Apellidos"].' ('.$datos[$d]["Rango"].')('.$datos[$d]["textos"].')('.$datos[$d]["Horas"].')</li>';
  }
};?>
